...
class ForecastDashboardView(TemplateView):
    ...
    def get_forecast_demand(self, demand):
        ...
        return demand_instance,block_values_dict
    
    # NEW: ---> Update demand in cache
    def update_forecast_demand(self,demand,block_values):
        demand = self.get_forecast_demand(demand)
        temp_dict = dict(demand.__dict__.items())
        temp_dict.pop('_state',None)
        temp_dict.update(**block_values)
        demand = Forecast_Master(**temp_dict)
        cache.set(str(self.request.user), demand, 3600)
        return demand
    ...
    def get_context_data(self, **kwargs):
        ...
        elif "ftype_action" in self.request.GET:
        ...
        if self.request.GET.get('fromBlock') and  self.request.GET.get('fromBlock') is not None :
            ...
            if action == "Add":
                ...
                self.update_forecast_demand(demand,block_values_dict) # NEW:
